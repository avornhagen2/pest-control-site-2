    ...
    <div className="border-t border-white/10 py-24 px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative grid md:grid-cols-3 gap-6">
          ...
          <div
            aria-hidden
            className="hidden md:block absolute top-[52px] left-[calc(16.66%+24px)] right-[calc(16.66%+24px)] h-px border-t border-dashed border-white/10"
          />
          ...
          {STEPS.map(({ number, Icon, title, description }, i) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="relative flex flex-col items-center text-center px-6 py-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm"
            >
              {/* Step icon */}
              <div className="relative mb-6">
                <div className="h-14 w-14 rounded-2xl border border-green-500/30 bg-green-500/10 flex items-center justify-center">
                  <Icon className="h-6 w-6 text-green-400" strokeWidth={1.5} />
                </div>
                <span className="absolute -top-2 -right-2 text-[10px] font-bold text-green-400/60 bg-black border border-green-500/20 rounded-full h-5 w-5 flex items-center justify-center leading-none">
                  {number.slice(1)}
                </span>
              </div>
