...
const FOLDER = process.argv[2] ?? "house-to-ants";
const INPUT_DIR = path.join("public", "frames", FOLDER);
const OUTPUT_DIR = path.join("public", "frames", `${FOLDER}-upscaled`);
...
async function upscaleFrame(frameName) {
...
async function main() {
  ...
  console.log(`Upscaling ${frames.length} frames from "${FOLDER}" → "${FOLDER}-upscaled"`);
